...
def arr_from_img(im,shift=0):
    ...
    return np.asarray(arr, dtype=np.float32).reshape((h,w,int(c))).transpose(2,1,0) / 255. - shift
...
def generate_moving_mnist(shape=(64,64), seq_len=30, seqs=10000, num_sz=28, nums_per_image=2):
    ...
    dataset = np.empty((seq_len,seqs, width, height), dtype=np.uint8)
    for seq_idx in range(seqs):
        ...
        for frame_idx in range(seq_len):
            ...
            for i,canv in enumerate(canvases):
                ...
                #print("tuple:",tuple(map(lambda p: int(round(p)), positions[i])))
            ...
            dataset[frame_idx,seq_idx] = (canvas * 255).astype(np.uint8).clip(0,255)
    return dataset

def main(dest, filetype='npz', frame_size=64, seq_len=20, seqs=10000, num_sz=28, nums_per_image=2):
    dat = generate_moving_mnist(shape=(frame_size,frame_size), seq_len=seq_len, seqs=seqs,num_sz=num_sz, nums_per_image=nums_per_image)
    print(dat.shape)
