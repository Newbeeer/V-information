...
for i in range(5):
    ...
    for j in range(0, 20):
        ...
        train_dataset_pair = Moving_MNIST_Frame(0, j)
        ...
        for epoch in range(1):
            ...
            for batch_idx, (x, y) in enumerate(tqdm(train_loader_pair)):
                ...
                scheduler.step()
        print('j {}, H loss : {}'.format(j, train_loss / len(train_loader_pair.dataset)))
        mat[0, j] = train_loss / len(train_loader_pair.dataset)
        b[j] = train_loss / len(train_loader_pair.dataset)
    np.save('frames_H_fixed_'+str(i)+'.npy', b)

'''
np.save('mat_2.npy',mat)

mat = np.load('mat_2.npy')
node_num = 5
g = adj2dict(mat)
